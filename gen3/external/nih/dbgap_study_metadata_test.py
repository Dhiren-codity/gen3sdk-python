...
def dbgap_metadata_instance():
    return dbgapStudyMetadata(api="http://mockapi.com")
...
def test_get_metadata_for_ids(mock_get, dbgap_metadata_instance, ids, mock_response, expected):
...
def test_get_metadata_for_ids_json_decode_error(mock_get, dbgap_metadata_instance):
    mock_get.return_value.json.side_effect = JSONDecodeError("Expecting value", "", 0)
    result = dbgap_metadata_instance.get_metadata_for_ids(["phs000004.v1"])
    assert result == {}
@patch('dbgapStudyMetadata.requests.get')

def test_get_metadata_for_ids_no_version_warning(mock_get, dbgap_metadata_instance, caplog):
    mock_get.return_value.json = MagicMock(return_value={"data": {"study": "metadata"}})
    dbgap_metadata_instance.get_metadata_for_ids(["phs000005"])
